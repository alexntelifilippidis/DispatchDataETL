...
async def test_data_loader_mysql_csv():
    ...
    await mysql_data_reader.load_data_to_db(
        data=data, table_name="source_csv", creation_columns=conf.creation_column_csv, chunk_size=conf.chunk_size
    )
...
async def test_data_loader_mysql_dat():
    ...
    await mysql_data_reader.load_data_to_db(
        data=data, table_name="source_dat", creation_columns=conf.creation_column_dat, chunk_size=conf.chunk_size
    )
    actual_data = await query_and_delete_data(
        host=conf.host,
        port=conf.port,  # Your MySQL port
